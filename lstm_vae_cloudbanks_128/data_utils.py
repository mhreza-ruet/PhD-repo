...
MULTI_TOKENS = [
    "[N+]", "[N-]", "[NH2+]", "[NH+]", "[nH+]", "[Na+]", "[Ca2+[", "(=O)", "[O-]",          # common charged species
    "Br", "Cl", "Si", "Na", "Li", "Ca", "Mg",                                   # multi‑letter elements
    "[C@H]", "[C@@H]"]                                                          # stereochemistry
...
def create_vocabulary(file_paths, test_smiles=None):
    """Build token ↔ index dictionaries from CSV files (column 'smiles')."""
        ...
        for smi in df["smiles"]:
            for tok in tokenize_smiles(smi):
                if tok not in token_to_idx:
                    new_idx = len(token_to_idx)
                    token_to_idx[tok] = new_idx
                    idx_to_token[new_idx] = tok

    # ensure coverage for test set
